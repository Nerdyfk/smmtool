...
class AdminManager {
  ...
  // Consistent with main app (ToolkityApp)
  // ==========================================
  setupTheme() {
    ...
    if (themeToggleBtn) {
      themeToggleBtn.addEventListener('click', () => {
        ...
        window.toolkityApp?.showToast('Theme Changed', `Switched to ${nextTheme} mode`, 'info');
      });
    }

    // Keep in sync with main app or other tabs
  ...
  changeOrderStatus(orderId, newStatus) {
    ...
    // Trigger Real-time Telegram Notifications
    if (newStatus === 'Completed') {
      window.dispatchEvent(new CustomEvent('order:completed', { detail: order }));
    } else if (typeof window.dispatchTelegramOrderNotification === 'function') {
      window.dispatchTelegramOrderNotification(order, newStatus);
    }
  }
