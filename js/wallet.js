...
class WalletManager {
  ...
  setupChains() {
    ...
    if (!this.selectedCoin || !wallets[this.selectedCoin]) {
      this.selectedCoin = keys[0] || 'USDT-ARB';
    }
  ...
  startDepositObserver() {
    ...
    if (method === 'bangla-qr') {
      ...
      const wallets = this.getCryptoWallets();
      const wallet = wallets[this.selectedCoin] || wallets['USDT-ARB'] || {};
      if (explorerTitle) explorerTitle.textContent = `${wallet.explorerName || 'Block Explorer'} (${wallet.chain || 'EVM'})`;
      if (explorerLink && wallet.explorer) {
        explorerLink.href = `${wallet.explorer}/tx/${txHash}`;
        explorerLink.textContent = `View on ${wallet.explorerName || 'Explorer'} ↗`;
      }
    }
