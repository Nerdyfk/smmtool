...
class ToolkityApp {
  ...
  setupTheme() {
    ...
    // Keep in sync if theme is changed in admin or other tabs
    window.addEventListener('storage', (e) => {
      if (e.key === 'toolkity_theme' && e.newValue) {
  ...
  setupMobileSidebar() {
    ...
    // Close when VIP modal or other buttons in sidebar are clicked
    const vipBtn = sidebar.querySelector('.sidebar-vip-btn');
    if (vipBtn) {
    ...
    // Touch swipe left on sidebar to dismiss
    let touchStartX = 0;
    let touchStartY = 0;
    ...
    sidebar.addEventListener('touchend', (e) => {
      if (e.changedTouches && e.changedTouches[0]) {
        ...
        // Dismiss if swiped left by 40+ pixels and not vertical scroll
    ...
    // Auto-dismiss on screen resize above mobile breakpoint
