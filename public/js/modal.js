...
class CryptoPaymentModal {
  ...
  getCryptoWallets() {
    return (typeof TOOLKITY_DATA !== 'undefined' && TOOLKITY_DATA.cryptoWallets) ? TOOLKITY_DATA.cryptoWallets : {};
  }
  ...
  setupCoinTabs() {
    ...
    const wallets = this.getCryptoWallets();
    const walletKeys = Object.keys(wallets);
    if (!this.selectedCoin || !wallets[this.selectedCoin]) {
      this.selectedCoin = walletKeys[0] || 'USDT-ARB';
    }
  ...
  updatePaymentDetails() {
    const wallets = this.getCryptoWallets();
    const wallet = wallets[this.selectedCoin] || wallets['USDT-ARB'] || {
      symbol: 'USDT',
      chain: 'Arbitrum',
      network: 'Arbitrum One (EVM L2)',
      gasEst: '~$0.02',
    ...
    if (networkBadge && wallet) {
      networkBadge.innerHTML = `<span style="color: var(--color-emerald);">●</span> ${wallet.network || wallet.chain}`;
    }
    if (gasEstLabel && wallet) {
      gasEstLabel.textContent = `Est. Gas: ${wallet.gasEst || '~$0.02'}`;
    }
    if (amountLabel && wallet) {
      amountLabel.textContent = `Send Exactly: ${price.toFixed(2)} ${wallet.symbol || 'USDT'}`;
    }
    if (summaryFollowers) {
    ...
    if (summaryPrice && wallet) {
      summaryPrice.textContent = `$${price.toFixed(2)} (${price.toFixed(2)} ${wallet.symbol || 'USDT'})`;
    }
  ...
  start1MinObserver() {
    ...
    if (titleEl) titleEl.textContent = `${wallet.explorerName || 'Block Explorer'} (${wallet.chain || 'EVM'} EVM)`;
    if (linkEl && wallet.explorer) {
      linkEl.href = `${wallet.explorer}/tx/${txHash}`;
      linkEl.textContent = `Open on ${wallet.explorerName || 'Explorer'} ↗`;
    }
